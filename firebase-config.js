...
try {
    ...
    db.enableNetwork().then(() => {
        console.log('Firestore 네트워크 연결됨');
    }).catch((err) => {
        console.error('Firestore 네트워크 연결 실패:', err);
    });
...
async function loadTransactions(retryCount = 0) {
    ...
    try {
        ...
        if (!networkStatus && retryCount < 3) {
            console.log(`네트워크 재연결 시도 (${retryCount + 1}/3)`);
        ...
        const snapshot = await db.collection('transactions')
            ...
            .get({ source: 'server' }); // 서버에서 직접 데이터 가져오기 시도
        ...
        // 오프라인 데이터 시도
        try {
            const offlineSnapshot = await db.collection('transactions')
                .orderBy('timestamp', 'desc')
                .get({ source: 'cache' });

            transactions = offlineSnapshot.docs.map(doc => ({
                id: doc.id,
                ...doc.data(),
                timestamp: doc.data().timestamp ? doc.data().timestamp.toDate() : new Date()
            }));

            console.log('오프라인 캐시에서 데이터를 불러왔습니다.');
            updateUI();
...
async function checkNetworkStatus() {
    try {
        await db.collection('transactions').limit(1).get({ source: 'server' });
        return true;
    } catch (error) {
        return false;
    }
